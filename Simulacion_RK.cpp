...
const int num_cer = 2;                      //numero de cerebros que seran simulados         
const int num_dat = 35000;                  //numero de datos 
double data[2*num_cer][num_dat];            //arreglo para almacenar los datos de cada cerebro
...
const double theta_objetivo = condiciones[0][1]; //angulo objetivo de los cerebros
...
double phi (int i, double t) {              // retorna el rumbo el cohete
    ...
    heading = heading + data[2*i_cerebro+1][i]/8;//retornar el valor de la direccion en cada instante
...
void runge_kutta_4 (double t_0, double r_0, double v_r0, double theta_0, double v_theta_0, double t_f, double dt, string nombre_archivo) {
    ...
    for (double t = t_0 ; t <= t_f; t = t + dt) {
        if (r_n < r_T) {
            fitness<< (m_0-m)*(m_0-m) + (theta_objetivo-theta_n)*(theta_objetivo-theta_n)*r_n*r_n<< "\n";
            break;
        }
        datos<<t<<","<<r_n<<","<<theta_n<<","<<data[2*i_cerebro+1][i]<<","<<heading<<"\n"; //DELETEME
...
int main() {
    leer_archivo_datos("datos.txt");

    leer_archivo_condiciones("condiciones.txt");
    ...
    for (i_cerebro = 0; i_cerebro < num_cer; i_cerebro ++){
        ...
        runge_kutta_4 (t_0, r_0, v_r_0, theta_0, v_theta_0, t_f, dt,"datos_runge_kutta"+to_string(i_cerebro)+".txt"); //ejecutar la sim
    }
    fitness.close();
